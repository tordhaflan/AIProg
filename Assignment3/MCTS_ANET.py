...
class MCTS_ANET:
    ...
    def __init__(self, game, state, anet_config, size, save_interval):
        ...
        self.size = size
        #self.ANET.load_model(250, 0.00005)

    def simulate(self, t, epsilon=0.2):
        ...
        if len(self.RBUF) > 1000:
            self.RBUF.reverse()
            new_buff = self.RBUF[:1000]
            save_RBUF(self.RBUF[1000:], self.size)
            self.RBUF = new_buff
            self.RBUF.reverse()
    ...
    def tree_search(self):
        ...
        if get_best_child(node, True).is_final_state:
            child = get_best_child(node, True, c)
            child.visits += 1
            return child, 1

        while is_expanded is True:
            node = get_best_child(node, minmax, c)
            node.visits += 1
            is_expanded = node.is_expanded
    ...
    def train(self, g):
        ...
        self.ANET.train(x_train, y_train, 50)
